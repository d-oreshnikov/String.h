...
#include "my_string.h"
#include "templates/my_sscanf_templates.h"
...
int my_sscanf(const char *str, const char *format, ...) {
  ...
  while (*format && work_func) {
    ...
    if (*format == *str && my_strchr(SPACE_SYM, *format) == my_NULL &&
        *format != '%') {
      format++;
      str++;
      continue;
    ...
    else if (my_strchr(SPACE_SYM, *format) != my_NULL) {
      my_delete_spaces(&format);
      my_delete_spaces(&str);
      continue;
    }
    /* реализация */
    else if (*format == '%') {
      ...
      format = my_reading_the_specifier(++format, &spec, &work_func);
      ...
      if (!*str && count_success == 0) {
        ...
        if (*str) {
          count_success += my_process_and_assign_str_val(start, &str, &spec,
                                                          &input, &work_func);
        }
      }
    }
...
void my_delete_spaces(const char **string) {
  ...
  while (my_strchr(SPACE_SYM, *p_str) != my_NULL && *p_str != '\0') p_str++;
...
const char *my_reading_the_specifier(const char *format, specifier *spec,
                                      bool *work_func) {
  /**
  ...
  if (is_digit(*format)) {
    ...
    while (is_digit(*format)) {
      flag.widht = flag.widht * 10 + my_ctoi(*format);
      format++;
    }
  }

  // Если есть параметр длины, то делай
  if (my_strchr("hLl", *format) != my_NULL) {
...
bool my_process_and_assign_str_val(const char *start, const char **str,
  ...
  switch (flag->spec) {
    case 'c':
      success = my_read_char(str, flag, input, work_func);
      break;
    case 's':
      success = my_read_string(str, flag, input);
      break;
    case 'd':
      if (flag->length == 'h')
        success = my_strtoi_s(str, flag, input, 10, work_func);
      else if (flag->length == 'l')
        success = my_strtoi_l(str, flag, input, 10, work_func);
      else if (flag->length == 'L')
        success = my_strtoi_ll(str, flag, input, 10, work_func);
      else
        success = my_strtoi_i(str, flag, input, 10, work_func);
      break;
    case 'u':
      if (flag->length == 'h')
        success = my_strtoi_us(str, flag, input, 10, work_func);
      else if (flag->length == 'l')
        success = my_strtoi_ul(str, flag, input, 10, work_func);
      else if (flag->length == 'L')
        success = my_strtoi_ull(str, flag, input, 10, work_func);
      else
        success = my_strtoi_ui(str, flag, input, 10, work_func);
      break;
    case 'i':
      if (flag->length == 'h')
        success = my_strtoi_s(str, flag, input, 0, work_func);
      else if (flag->length == 'l')
        success = my_strtoi_l(str, flag, input, 0, work_func);
      else if (flag->length == 'L')
        success = my_strtoi_ll(str, flag, input, 0, work_func);
      else
        success = my_strtoi_i(str, flag, input, 0, work_func);
      break;
    case 'o':
      if (flag->length == 'h')
        success = my_strtoi_us(str, flag, input, 8, work_func);
      else if (flag->length == 'l')
        success = my_strtoi_ul(str, flag, input, 8, work_func);
      else if (flag->length == 'L')
        success = my_strtoi_ull(str, flag, input, 8, work_func);
      else
        success = my_strtoi_ui(str, flag, input, 8, work_func);
      break;
    case 'x':
    case 'X':
      if (flag->length == 'h')
        success = my_strtoi_us(str, flag, input, 16, work_func);
      else if (flag->length == 'l')
        success = my_strtoi_ul(str, flag, input, 16, work_func);
      else if (flag->length == 'L')
        success = my_strtoi_ull(str, flag, input, 16, work_func);
      else
        success = my_strtoi_ui(str, flag, input, 16, work_func);
      break;
    case 'p':
      success = my_strtoi_ull(str, flag, input, 16, work_func);
      break;
    ...
    case 'G':
      if (flag->length == 'l')
        success = my_strtod(str, flag, input, work_func);
      else if (flag->length == 'L')
        success = my_strtold(str, flag, input, work_func);
      else
        success = my_strtof(str, flag, input, work_func);
      break;
    case 'n':;
...
bool my_read_string(const char **str, specifier *flag, va_list *input) {
  ...
  while (my_strchr(SPACE_SYM, *p_str) == my_NULL && *p_str &&
         flag->widht != 0) {
