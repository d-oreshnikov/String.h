#ifndef SRC_my_SSCANF_TEMPLATES_H_
#define SRC_my_SSCANF_TEMPLATES_H_

#define DECLARE_NO_FLOAT_FUNC(type, c, signed_or_not, ll_u_or_not)             \
  bool my_strtoi_##c(const char **str, specifier *flag, va_list *input,       \
    ...
    ll_u_or_not next_num = 0, num = 0;                                         \
                                                                               \
    int sign = my_get_sign(&flag->widht, str);                                \
    const char *p_str = *str;                                                  \
                                                                               \
    if (base == 0) base = my_get_base(*str);                                  \
    ...
    while (my_strchr(num_sys, *p_str) != my_NULL && flag->widht && *p_str) { \
      num = next_num;                                                          \
      next_num = num * base + my_ctoi(*p_str);                                \
      if (next_num < num)                                                      \
        overload = true;                                                       \
      else                                                                     \
        num = next_num;                                                        \
...
#define DECLARE_FLOAT_FUNC(type, c)                                    \
  bool my_strto##c(const char **str, specifier *flag, va_list *input, \
                    bool *work_func) {                                 \
    ...
    const char *p_str = *str;                                          \
                                                                       \
    if (my_checkinf(flag->widht, str)) {                              \
      res_float = 1.0 / 0.0;                                           \
      p_str += 3;                                                      \
      res = 1;                                                         \
    } else if (my_checknan(flag->widht, str)) {                       \
      res_float = 0.0 / 0.0;                                           \
      p_str += 3;                                                      \
      res = 1;                                                         \
    } else {                                                           \
      if (is_digit(*p_str) && *p_str) {                                \
        ...
        while (is_digit(*p_str) && *p_str && flag->widht) {            \
          res_float = res_float * 10 + my_ctoi(*p_str);               \
          p_str++;                                                     \
          flag->widht--;                                               \
        }                                                              \
      }                                                                \
      ...
      if ((*p_str == 'e' || *p_str == 'E') && *work_func) {            \
        ...
        p_str++;                                                       \
        flag->widht--;                                                 \
        int exp_neg = my_get_sign(&flag->widht, &p_str);              \
        bool overload = false;                                         \
        if (is_digit(*p_str)) {                                        \
          while (is_digit(*p_str) && flag->widht && *p_str) {          \
            int next = exp_pow;                                        \
            next = exp_pow * 10 + my_ctoi(*p_str);                    \
            if (next < exp_pow) {                                      \
              exp_pow = -1;                                            \
              overload = true;                                         \
            } else {                                                   \
